...
private static HashSet<String> allClaimSet = new HashSet<>();
private static HashSet<String> overlapSet = new HashSet<>();
...
public static void main(String[] args)
    ...
    allClaimSet.removeAll(overlapSet);

    for (String intact : allClaimSet) {
...
static void draw(String claimID, String X, String Y, String width, String height) {
	...
	for (int y = Integer.valueOf(Y); y < Integer.valueOf(Y) + Integer.valueOf(height); y++) {
	    if (!fabric[x][y].contains(".")) {
		overlapSet.add(fabric[x][y]);
		overlapSet.add(claimID);
		fabric[x][y] = claimID;
	    }
	    else {
		fabric[x][y] = claimID;
